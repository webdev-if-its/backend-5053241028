...
func HitungSubtotal(qty int, hargaSatuan float64) float64 {
	var quant float64 = float64(qty)
	var result float64 = quant * hargaSatuan
	return result
}
...
func ValidasiPesanan(qty []int, hargaSatuan []float64) (bool, string) {
	...
	if len(qty) != len(hargaSatuan) {
		status = false
		return status, "panjang qty dan hargaSatuan tidak sama"
	}
	for i := 0; i < len(hargaSatuan); i++ {
		if hargaSatuan[i] < 0 {
			status = false
			return status, "hargaSatuan tidak boleh negatif"
		}
		if qty[i] <= 0 {
			status = false
			return status, "qty tidak boleh nol/negatif"
		}
	}
	status = true
	return status, ""
...
func RingkasanPesanan(qty []int, hargaSatuan []float64, tarifPajak float64) string {
	...
	totalSetelahDiskon := TotalSetelahDiskon(qty, hargaSatuan, tarifPajak)
	status := TentukanStatus(totalSetelahDiskon)
	
	return fmt.Sprintf("Subtotal: %.2f\nDiskon: %.2f\nTotal Setelah Diskon: %.2f\nStatus: %s", subtotal, diskon, totalSetelahDiskon, status)
}
